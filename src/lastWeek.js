...
function onResults(results) {
  if (results.multiHandLandmarks.length > 0) {
    ...
    if (previousCategory !== category) {
      switch (category) {
        case "Pinch":
          camAlterMiddleWare = dragMode(landmarks, camPos);
          break;
        case "Rock":
          camAlterMiddleWare = zoomMode(landmarks, camPos);
          break;
        default:
    ...
    if (camAlterMiddleWare !== undefined) {
      camPos = camAlterMiddleWare(landmarks, camPos);
    }
...
function dragMode(initLandMark, initCamPos) {
  const tiltOrigin = [initLandMark[9].x, initLandMark[9].y, initLandMark[9].z]; //middle of the hand
  const camBeforeTilt = [...initCamPos];
  return (landmarks, camera) => {
    ...
    camera = rotateVectorAroundAxis(camBeforeTilt, axis, dy);
    camera = rotateVectorAroundAxis(camera, [0, 0, 1], dx);
...
function zoomMode(initLandMark, initCamPos) {
  const referencePoint = [initLandMark[9].x, initLandMark[9].y, initLandMark[9].z] // middle of the hand
  const camBeforeTransform = [...initCamPos];
  return (landmarks, camera) => {
    const [x, y, z] = [landmarks[9].x, landmarks[9].y, landmarks[9].z]; // middle of the hand
    const [x_r, y_r, z_r] = referencePoint;
    return extendVector3D(camBeforeTransform, 20*(x-x_r))
  }
};
...
const videoStreamStarted = new Promise((resolve) => videoElement.addEventListener("loadedmetadata", resolve))
